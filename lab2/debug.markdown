...
1. **启动QEMU调试服务器**：
   ```bash
   /home/wekolss/qemu-4.1.1/riscv64-softmmu/qemu-system-riscv64 -S -s -nographic -machine virt -kernel ucore.bin
   ```

2. **连接GDB到QEMU**：
   ```bash
   gdb /home/wekolss/qemu-4.1.1/riscv64-softmmu/qemu-system-riscv64
...
3. **在ucore中设置断点**：
   ```bash
   riscv64-unknown-elf-gdb ucore.bin
   (gdb) target remote :1234
   (gdb) b 0x1000  # 在ucore中的特定地址设置断点
   (gdb) c
   ```
