...
在本实验的执行过程中创建且运行了2个内核线程。
第 0 个内核线程 idleproc，是一个在操作系统中常见的概念，用于表示空闲进程。在操作系统中，空闲进程是一个特殊的进程，它的主要目的是在系统没有其他任务需要执行时，占用 CPU 时间，同时便于进程调度的统一化。
通过 kernel_thread 函数来实现第一个内核线程的创建。这个函数负责为新的内核线程创建一个初始化好的中断帧，并通过调用 do_fork 函数将其转化为一个新的进程。在实验中，这个子内核线程的工作就是输出一些字符串，然后返回。
